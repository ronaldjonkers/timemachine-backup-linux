...
server_install_dependencies() {
    ...
    case "${os}" in
        ubuntu|debian|linuxmint|pop|elementary|zorin)
            ...
            apt-get install -y -qq rsync openssh-server socat curl mailutils postfix gnupg2 tar zip python3 2>/dev/null || true
            step_done "apt packages installed"
            ;;
        centos|rhel|rocky|almalinux|ol)
            info "Using yum..."
            yum install -y -q rsync openssh-server socat curl s-nail postfix gnupg2 tar zip python3 2>/dev/null || {
        ...
        fedora)
            info "Using dnf..."
            dnf install -y -q rsync openssh-server socat curl s-nail postfix gnupg2 tar zip python3 2>/dev/null || {
        ...
        opensuse*|sles|suse)
            info "Using zypper..."
            zypper --non-interactive install rsync openssh socat curl mailx postfix gpg2 tar zip python3 2>/dev/null || true
            step_done "zypper packages installed"
            ;;
        arch|manjaro|endeavouros)
            info "Using pacman..."
            pacman -Sy --noconfirm --needed rsync openssh socat curl postfix gnupg tar zip python 2>/dev/null || true
            step_done "pacman packages installed"
            ;;
        alpine)
            info "Using apk..."
            apk add --no-cache rsync openssh socat curl postfix gnupg mailx tar zip python3 2>/dev/null || true
        ...
        *)
            ...
            if command -v apt-get &>/dev/null; then
                ...
                dnf install -y -q rsync openssh-server socat curl s-nail postfix gnupg2 tar zip python3 2>/dev/null || true
            elif command -v yum &>/dev/null; then
                yum install -y -q rsync openssh-server socat curl s-nail postfix gnupg2 tar zip python3 2>/dev/null || true
    ...
    # Enable and start postfix if installed (needed as local MTA for s-nail/mailutils)
    if command -v postfix &>/dev/null && command -v systemctl &>/dev/null; then
        systemctl enable postfix 2>/dev/null || true
        systemctl start postfix 2>/dev/null || true
        info "Postfix MTA enabled and started"
    fi
}
...
reconfigure_server() {
    ...
    step 2 ${total} "Setting file permissions & symlinks"
    server_setup_permissions
