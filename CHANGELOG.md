...
### Changed
- `bin/dump_dbs.sh`: prefer `mariadb` over the deprecated `mysql` client (and `mariadb-dump` over `mysqldump`) on modern MariaDB 11.x systems. Falls back to `mysql`/`mysqldump` when only those are installed.
- Added trailing newline to the inline `[client]` `--defaults-extra-file` content used for `SHOW DATABASES` and `mysqldump`. Some MariaDB client versions reject a missing terminator on the final `password=` line.

### Reverted
- The v3.7.9 large refactor of `bin/dump_dbs.sh` (multi-method login discovery, dump fallback, new env vars) was rolled back per request. Only the minimal MariaDB-client-preference change and the cnf-newline fix remain.
...
### Fixed
- **"DB Interval OK" emails persist after update** — Root cause: `tmctl update` did not forcefully kill old `tmserviced.sh` processes before restarting. The old daemon kept running in memory with pre-v3.3.2 code that sent "DB Interval OK" notifications. The reconfigure step now force-stops the service, kills any orphaned `tmserviced.sh` processes, and starts fresh. Also added non-systemd restart fallback.
...
### Improved
- **Documentation overhaul** — Updated Help page and README.md with all features from v3.6.5–3.6.9:
  - Database version browser: two-step drill-down flow (pick date/time → see individual files)
  - Per-database download/restore with modification timestamps
  - Per-server DB compression (`--db-compress` / `--no-db-compress`)
  - Updated REST API endpoint table with all new endpoints
  - Updated dump directory structure showing interval version subdirectories
  - Fixed "Details" → "Snapshots" references in help page
...
### Fixed
- **Test servers.conf accidentally committed** — Removed `config/servers.conf` from git tracking. This file is unique per installation and should never be in the repository. Added to `.gitignore` to prevent recurrence.

## [3.6.3] - 2026-02-17

### Fixed
- **Disk usage always showed `-- / -- (0%)`** — `df -h` output parsing failed on wrapped filesystem names. Now uses `df -Ph` (POSIX single-line mode) and parses fields from the right side, which works regardless of filesystem name length.
...
### Fixed
- **Rotation ran after db-only backups** — Old backup cleanup (rotation) now only runs after full or files-only backups. DB-only backups skip rotation entirely, preventing unnecessary (and failing) delete attempts.
- **Rotation permission denied** — `rm -rf` in `tm_rotate_backups` now uses `sudo rm -rf`. Backup files are owned by root (rsync preserves remote ownership), so the `timemachine` user needs sudo to delete them.
...
### Improved
- **Redis backup is now opt-in** — Redis is skipped unless a credential file exists at `/home/timemachine/.credentials/redis.conf` or `redis.pw`. To enable: `touch /home/timemachine/.credentials/redis.conf` on the remote server.

## [3.4.3] - 2026-02-17

### Improved
- **MySQL password fallback to `/root/mysql.pw`** — If the configured password file (`/home/timemachine/.credentials/mysql.pw`) is empty or missing, `dump_dbs.sh` now tries `/root/mysql.pw` as fallback before reporting an error.
